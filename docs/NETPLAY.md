...
1. ✅ 服务端自测：5 模拟客户端整局（含刺杀）与 core.js 同种子直跑 parity、全程隐私断言、
   ...
   另加 `avalon-cs/NetSmoke`：真实 NetSession（Unity 同一份源码）× 本地/公网 node 服整局烟雾测试通过。
2. ⏳ 真机联机：服务器已部署（`ws://114.55.134.216:8080`，systemd 常驻）。单机验证可用**机器人补位**：
   手机建房开局后，PC 上 `cd avalon-cs/NetSmoke && dotnet run -- ws://114.55.134.216:8080 join 房间码 4`
   即有 4 个机器人进房陪打整局（每人回合靠服务器超时自动行动兜底）。两台真机互打仍待第二台设备。
